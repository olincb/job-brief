...
def users_to_run(rows, only_users):
    ...
    Email matching is case-insensitive; everyone excluded here is a non-send day."""
    allowed = {email.lower() for email in only_users} if only_users else None
    return [
        row for row in rows
        if row.get("active") == "yes" and (allowed is None or row.get("email", "").lower() in allowed)
    ]
