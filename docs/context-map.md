...
- `app.py` — the Flask app: the `Allowed` lookup and the gate, the operator's invite notice, the routes, the signup stash between the form and the Drive consent, and the writes that make a user's sheet and registry row. The only web module that reads the environment.
- `form.py` — the signup form: the lists the questionnaire offers back to the user, and one `Answers` row from a submitted form.
...
- `templates/` — `base.html` and the four pages: `signin.html`, `invite.html`, `signup.html` (the questionnaire), `settings.html`.
...
- `test_web.py` — sign-in scopes, the gate (allowed, operator, unknown address), the invite email, state and verification refusals, and signup end to end: submit, the Drive callback's sheet and registry writes, a lost stash, a model failure.
