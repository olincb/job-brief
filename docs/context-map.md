...
- `test_web.py` — sign-in scopes, the gate (allowed, operator, unknown address, an `Allowed` row deleted under a live session), the invite email, state and verification refusals, sign-out, signup end to end (submit, the Drive callback's sheet and registry writes, a lost stash, a model failure), and settings: what the page shows, what a save writes, pause, retake, delete-me.
