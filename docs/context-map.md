...
- `test_run.py` — each frequency on a send and a non-send day; the lookback stretched by a gap and defaulted with no email on record; and the loop over fake sheets: one user's failure leaves the next served, `ONLY_USERS`, a non-send day, an empty filter, the heartbeat either side of its threshold, and every source skipped.
