...
def test_filters_widen_level_words_and_exclude_entry_titles():
    filters, excludes = title_filters(ANSWERS, ROLE_TITLES)
    assert filters == ["water quality analyst", "senior hydrologist", "associate hydrologist", "specialist hydrologist"]
    assert excludes == ["sales", "intern", "junior"]


def test_entry_level_user_keeps_the_entry_titles():
    answers = dict(ANSWERS, experience="1 year, plus a summer internship", search_titles="junior hydrologist")
    filters, excludes = title_filters(answers, ROLE_TITLES)
    assert filters == ["junior hydrologist", "intern hydrologist"]
...
def test_draft_profile_attaches_a_pdf_resume_and_returns_the_settings(monkeypatch):
    ...
    assert "Never copy contact details" in parts[1]["text"]
    assert profile == "## Summary\n\nA water person."
    assert settings["title_filter"].splitlines()[0] == "water quality analyst"
    assert settings["title_exclude"].splitlines() == ["sales", "intern", "internship", "trainee", "apprentice", "junior", "entry level"]
